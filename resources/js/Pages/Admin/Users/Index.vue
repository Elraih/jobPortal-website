      ...
      <Table>
        ...
        <template v-slot:tableBody>
          <tr v-for="jobSeeker, index in jobSeekers.data">
            ...
            <td class="px-4 py-2" v-if="jobSeeker.user_profile.full_name"> <Link class="text-blue-600 hover:underline" :href="route('admin.user.show', {user:jobSeeker.id})">{{jobSeeker.user_profile.full_name }}</Link></td>
            ...
            <td>
              <form @submit.prevent="deleteJobseeker(jobSeeker.id)">
...
import { useForm } from '@inertiajs/vue3';
  ...
  export default {
    ...
    methods: {
      deleteJobseeker(id){
       ...
       filter(){
    this.form.get(route('admin.user.index'), {
      preserveState: true,
      preserveScroll: true,
      replace: true,
    });
  ...
  clearFilter(){
    if(this.form.jobSeekerName != ''  ){
      ...
      this.$refs.fil.reset();
      this.$inertia.get(route('admin.user.index'));
    }
    },
    },
    mounted(){
